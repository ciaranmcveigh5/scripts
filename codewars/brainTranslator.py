...
def removeInvalidStatementsFromCode(code):
    for invalidStatement in invalidStatements:
        code = code.replace(invalidStatement, '')
    return code

def removeInvalidCharacters(code):
    result = ''.join(c for c in code if c in validCharacters)
    return result

def convertToC(code):
    result = removeInvalidCharacters(code)
    result = removeInvalidStatementsFromCode(result)
    return result


class TestMethods(unittest.TestCase):

    def test_invalid_statement_removal(self):
        self.assertEqual(convertToC('+-<>[]'), '')
        self.assertEqual(convertToC('+-<{>a[s]'), '')
